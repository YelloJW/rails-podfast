...
sport_img_url_array = ["https://uploads.guim.co.uk/2018/08/06/football-weekly-3000_(1).jpg",
                       ...
                       "https://ichef.bbci.co.uk/images/ic/624x351/p06q9211.jpg",
                       "https://ichef.bbci.co.uk/images/ic/624x351/p06q9211.jpg",
...
Episode.create(listens: rand(1..2000), img_url: sport_img_url_array[4],title: "Ep. 95 - Chicken Leg, Chicken Wing", summary: "Fresh news from the Gangs of the EPL, a Managers Lunch Club meeting, the Guru gets festive, Andy gets booted into his own car, and we receive two brand new WAVs", length: rand(1..180), upvotes: rand(1..1000), url: sport_url_array[4])
Episode.create(listens: rand(1..2000), img_url: sport_img_url_array[5],title: "Ep. 91 - Longy Special", summary: "Peter Beardsley returns, Neil Hunt faces a pie-eating adversary, and there’s some singing, some quizzing, and a magical gold card…", length: rand(1..180), upvotes: rand(1..1000), url: sport_url_array[5])
Episode.create(listens: rand(1..2000), img_url: sport_img_url_array[6],title: "Episode 16 - Foden's X-Factor & Pure Filth", summary: "We're joined by Ben Foden to chat Nicole Scherzinger and Thom Evans and all things X-Factor as well as life in New York and a bit of rugby too. And, we dissect all the Champions Cup action with an open cloth, including some absolute filth of the worst kind at the AJ Bell Stadium and some filth of the best kind from Finn Russell!", length: rand(1..180), upvotes: rand(1..1000), url: sport_url_array[6])
Episode.create(listens: rand(1..2000), img_url: sport_img_url_array[7],title: "Arsecast Extra Episode 319 - 25.11.2019", summary: "On this week's show we recap the 2-2 draw with Southampton, touching on Unai Emery's tactics, formation changes and substitutions. We wonder why it is we should have been so afraid of a team struggling in the relegation zone, and also why did a last gasp equaliser feel so empty? Emery's future is an ongoing issue, but we turn some focus on the decision makers at the club and ask why are they allowing this to continue? We chat about potential replacements, including Allegri, Arteta, Ljungberg and Vieira, ponder the January transfer window, rate our faith in club officials and lots more in this bumper but quite depressing episode. If you stick with it all the way through, we thank you!", length: rand(1..180), upvotes: rand(1..1000), url: sport_url_array[7])
...
Episode.create(listens: rand(1..2000), img_url: comedy_img_url_array[3], title: "S2E2 - 'Amsterdammmm'", summary: "Belinda makes her first trip to Europe to secure a lucrative deal with Peter Rouse. Before she meets him, she encounters some new, interesting characters with their own sexual agendas...", length: rand(1..180), upvotes: rand(1..1000), url: comedy_url_array[3])
Episode.create(listens: rand(1..2000), img_url: comedy_img_url_array[4], title: "Ep 40. Cheese table", summary: "It's episode 40 and Chris is off to Blackpool! As well as talking all things Strictly, Rosie and Chris discuss Sandra's latest stash and Rosie's recent trip to London. There are some brilliant stories and questions from the public, topics range from an unfortunate cheese incident to a trip to A&E on a third date. And of course there is a celeb question - this week it's from Strictly alumni Mike Bushell.", length: rand(1..180), upvotes: rand(1..1000), url: comedy_url_array[4])
Episode.create(listens: rand(1..2000), img_url: comedy_img_url_array[5], title: "No Art Just Artists, Fake Quotes, Motivational Speakers, Everybody Flirts, VR - 171", summary: "The 3ShotsOfTequila Podcast - Episode 171 - Topics: No Art Just Artists, Fake Quotes, Motivational Speakers, Everybody Flirts, VR + MoreFOLLOW US ON INSTAGRAM TO KEEP UP-TO-DATE WITH ALL THINGS 3HOTS...Instagram: @3ShotsOfTequila_HOSTS:Marvin AbbeyTwitter: @MarvAbbey Instagram: @MarvinAbbeyEmail: MarvinAbbey@icloud.comKeith DubeTwitter: @MrExposedInstagram: @MrExpsdEmail: hello@keithdube.coTazer BlackTwitter: @TazerBlackInstagram: @TazerBlackEmail: TazerX@mail.comSPECIAL GUEST:Gracious KayWant us to read out your dilemma?Have a question for the team?Wanna get in touch with us?Email: 3ShotsOfTequila@mail.com", length: rand(1..180), upvotes: rand(1..1000), url: comedy_url_array[5])
Episode.create(listens: rand(1..2000), img_url: comedy_img_url_array[6], title: "The Now Show - Series 55 Episode 5", summary: "Download the best satirical comedy from Radio 4, every Friday.", length: rand(1..180), upvotes: rand(1..1000), url: comedy_url_array[6])
Episode.create(listens: rand(1..2000), img_url: comedy_img_url_array[7], title: "#12 - What Really Happened At Zane's Birthday Party", summary: "Zane went missing at his own birthday and was later found with a lot of rumors to clear up! Who needs a shot of espresso when you have and episode this good?", length: rand(1..180), upvotes: rand(1..1000), url: comedy_url_array[7])
...
Episode.create(listens: rand(1..2000), img_url: comedy_img_url_array[9], title: "404 - Jan Pieterszoon Coen (live)", summary: "Comedians Dave Anthony and Gareth Reynolds examine Jan Pieterszoon Coen of the Dutch East India Company. Recorded live in Amsterdam.Tour DatesSourcesRedbubble Merch", length: rand(1..180), upvotes: rand(1..1000), url: comedy_url_array[9])
...
comedy_news_episode = Episode.new(listens: rand(1..2000), img_url: comedy_news_img_url, title: "3 Shots of Tequila", summary: "The 3 Shots Of Tequila Podcast is a fun and light heartedpodcast about any and everything, underneath the sun. From sex and relationships to religion and current affairs.", url: comedy_news_url, length: rand(1..180), upvotes: rand(1..1000))
